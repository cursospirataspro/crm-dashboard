...
async function fetchApi(r) {
  ...
  try {
    ...
    do {
      ...
      // Mostrar progreso si hay mÃºltiples pÃ¡ginas
      if (totalPages > 1) {
...
function range() {
  ...
  if (preset === "custom") {
    ...
    from = new Date(now.getFullYear() - 5, 0, 1, 0, 0, 0); // mÃ¡ximo 5 aÃ±os atrÃ¡s
    to   = new Date(now); to.setHours(23,59,59,999);
  } else {
...
// =============================================================
// Igual que WooCommerce Analytics: solo completed + processing cuentan como venta vÃ¡lida
function validRevenueOrders(o) {
  return o.filter(x => ["completed","processing"].includes(statusNorm(x.status)));
}

// Ingresos netos = subtotal - descuentos (sin impuestos ni envÃ­o), igual que WooCommerce Net Sales
...
function renderRecentFeed() {
  ...
  const statusIcon = s => ({ completed:'âœ…', processing:'Ã¯Â¿Â½Ã‚Âï¿½', pending:'ðŸ””', cancelled:'Ã¯Â¿Â½Ã‚Âï¿½', refunded:'â†©', 'on-hold':'Ã¯Â¿Â½Ã‚Âï¿½' }[s] || 'ðŸ“¦');
  const timeAgo = d => {
...
function renderWeekdayChart() {
  ...
  $("#orderSummary").innerHTML = [
    ['âœ… Completados', paid],
    ['Ã¯Â¿Â½Ã‚Âï¿½ Pendientes',  pend],
    ['Ã¯Â¿Â½Ã‚Âï¿½ Cancelados',  canc],
...
function rfmScore(c, allCustomers) {
  ...
  return              { score: total, label: 'Ã¯Â¿Â½Ã‚Âï¿½ Inactivo',    cls: 'rfm-inactive' };
}
...
function drawGlobe() {
  ...
  ctx.textAlign = 'left';
  ctx.fillText('Ã¯Â¿Â½Ã‚Â PaÃ­ses con ventas activas', 16, h - 28);
...
// GRÃ¯Â¿Â½Ã‚ÂFICO DE LÃ¯Â¿Â½Ã‚ÂNEA â€” ingresos por dÃ­a
// =============================================================
...
function renderRfmScatter() {
  ...
  const colors = { 'Ã¯Â¿Â½Ã‚Â VIP':'#fbbf24', 'ðŸ”¥ Activo':'#ef233c', 'âš¡ Potencial':'#38bdf8', 'ðŸ˜´ En riesgo':'#f59e0b', 'Ã¯Â¿Â½Ã‚Âï¿½ Inactivo':'#9ca3af' };
  ...
  ['new','contacted','won','lost'].forEach(col => {
    ...
    list.innerHTML = cols[col].map(({ id, lead }) => {
      ...
      const mvBtns = otherCols.map(c => {
        const labels = { new:'Nuevo', contacted:'Contactado', won:'âœ… Ganado', lost:'Ã¯Â¿Â½Ã‚Âï¿½ Perdido' };
...
function kanbanMove(id, newStatus) {
  if (KANBAN_STATE[id]) {
    ...
    saveKanban();
    renderKanban();
    const labels = { new:'Nuevo', contacted:'Contactado', won:'Ganado âœ…', lost:'Perdido Ã¯Â¿Â½Ã‚Âï¿½' };
...
function openCustomerModal(emailOrName) {
  ...
  const stIcon = s => ({ completed:'âœ…', processing:'Ã¯Â¿Â½Ã‚Âï¿½', pending:'ðŸ””', cancelled:'Ã¯Â¿Â½Ã‚Âï¿½', refunded:'â†©' }[s] || 'ðŸ“¦');
...
function bind() {
  ...
  $("#cfgShowToken")?.addEventListener("click", () => {
    ...
    $("#cfgShowToken").textContent = inp.type === "password" ? "Ã¯Â¿Â½Ã‚Â Ver" : "ðŸ™ˆ Ocultar";
  });
  ...
  $("#cfgTestBtn")?.addEventListener("click", async () => {
    ...
    try {
      ...
      if (res.ok && (data.orders !== undefined || data.total !== undefined)) {
        ...
        st.className = "cfg-status error"; st.textContent = "Ã¯Â¿Â½Ã‚Â Plugin no encontrado (404). Verifica que el plugin estÃ© activo.";
      } else {
        st.className = "cfg-status error"; st.textContent = "âš  Respuesta inesperada: " + res.status;
      }
    } catch (e) {
      st.className = "cfg-status error";
      st.textContent = "Ã¯Â¿Â½Ã‚Âï¿½ No se pudo conectar. Verifica la URL o CORS del servidor.";
    }
  });
...
// =============================================================
// HORA Ã“PTIMA DE ENVÃ¯Â¿Â½Ã‚ÂO
// =============================================================
...
async function exportPDF() {
  ...
  if (btn) { btn.textContent = "Ã¯Â¿Â½Ã‚Âï¿½ Generando PDF..."; btn.disabled = true; }
...
const PRES_VIEWS = [
  ...
  { id: "geo",          label: "Ã¯Â¿Â½Ã‚Â GeografÃ­a" },
];
let _presIdx     = 0;
let _presTimer   = null;
let _presSeconds = 10;
...
const WIDGET_DEFS = [
  ...
  { id: "view-geo",      label: "Ã¯Â¿Â½Ã‚Â GeografÃ­a",            sub: "Rankings de paÃ­ses y ciudades" },
];
...
const PP_COUNTRIES = {
  ...
  NL:"PaÃ­ses Bajos", PT:"Portugal", BE:"BÃ©lgica", CH:"Suiza", SE:"Suecia",
  IN:"India", CN:"China", JP:"JapÃ³n", AU:"Australia", CA:"CanadÃ¡",
...
// Mapa: codigo ISO numerico â†’ alpha-2
    ...
    ctEl.innerHTML = `
      <div style="display:flex;gap:16px;flex-wrap:wrap;align-items:flex-start">
        ...
        <div style="min-width:210px;padding:4px 0">
          <p style="font-size:11px;color:var(--muted);margin:0 0 10px 0;font-weight:700;text-transform:uppercase;letter-spacing:.5px">Top paÃ­ses</p>
          ${legend || '<p style="color:var(--muted);font-size:13px">Sin datos de paÃ­s aÃºn.</p>'}
        </div>
      </div>`;
  } catch (err) {
...
// Divide un rango de fechas en ventanas de maxDays dÃ­as
...
async function loadPaypalData() {
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      if (statusEl) statusEl.textContent = `Cargando... bloque ${i + 1}/${chunks.length} (${cFrom} â†’ ${cTo})`;
      const data = await ppFetchTransactions(cFrom, cTo, 1);
      const txns = data.transaction_details || [];
      allTxns.push(...txns);
      // Si hay mÃ¡s pÃ¡ginas en este bloque
    ...
    ppRenderAll(allTxns);
    if (statusEl) statusEl.textContent = `${allTxns.length} transacciones Â· ${new Date().toLocaleTimeString("es-PE")}`;
  } catch(err) {
...
function ppApplyDatePreset(days) {
  ...
  if (days === "all") {
    // PayPal Transaction Search API solo permite hasta 3 aÃ±os hacia atrÃ¡s
